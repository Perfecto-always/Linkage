import axios from "axios";
import { useState } from "react";
import Modal from "../../components/Modal";
import { BACKEND_URL } from "../../config/config";

function Setting() {
  const [isOpen, setIsOpen] = useState(false);
  const logout = () => {
    axios
      .post(BACKEND_URL + "/api/user/logout", null, {
        withCredentials: true,
      })
      .then((res) => {
        window.location.href = "/login";
        localStorage.removeItem("username");
      })
      .catch((res) => {
        window.location.href = "/login";
        localStorage.removeItem("username");
      });
  };

  return (
    <>
      <Modal open={isOpen} close={setIsOpen}>
        <p>Are you sure you want to logout?</p>
        <div className='text-right mt-3'>
          <button
            className='p-3 text-blue-400 hover:underline'
            onClick={() => setIsOpen(false)}
          >
            Cancel
          </button>
          <button
            className='text-white bg-red-500 py-2 px-4 rounded-md hover:bg-red-600'
            onClick={() => {
              logout();
            }}
          >
            Confirm
          </button>
        </div>
      </Modal>
      <div className='w-full flex justify-center items-center  h-screen'>
        <button
          onClick={() => setIsOpen(true)}
          className='bg-primary-light-100 inline-flex px-6 border border-gray-700 py-3 rounded hover:bg-opacity-90 hover:bg-red-500 transition-colors ease-in'
        >
          Logout
          <svg
            xmlns='http://www.w3.org/2000/svg'
            width='24'
            height='24'
            viewBox='0 0 24 24'
            fill='currentColor'
            className='ml-3'
          >
            <path d='m2 12 5 4v-3h9v-2H7V8z'></path>
            <path d='M13.001 2.999a8.938 8.938 0 0 0-6.364 2.637L8.051 7.05c1.322-1.322 3.08-2.051 4.95-2.051s3.628.729 4.95 2.051 2.051 3.08 2.051 4.95-.729 3.628-2.051 4.95-3.08 2.051-4.95 2.051-3.628-.729-4.95-2.051l-1.414 1.414c1.699 1.7 3.959 2.637 6.364 2.637s4.665-.937 6.364-2.637c1.7-1.699 2.637-3.959 2.637-6.364s-.937-4.665-2.637-6.364a8.938 8.938 0 0 0-6.364-2.637z'></path>
          </svg>
        </button>
      </div>
    </>
  );
}
export default Setting;
