...
const SideBar = () => {
  ...
  return (
    <nav>
      <div className='h-screen w-max flex flex-col justify-center space-y-10 bg-primary-400'>
        {liItems.map((items) => (
          <NavLink
            to={items.to}
            key={items.id}
            title={items.alt}
            activeClassName='bg-primary-light-0 filter-none'
            className='text-accent-0 p-2 hover:bg-primary-100 mx-2 rounded-md filter grayscale'
          >
            {items.img}
          </NavLink>
        ))}
